...
% Purpose: comprehensive evidence-led synthesis for ACP-RA + DAD
% =========================================================
...
\newcommand{\PaperSubtitle}{Comprehensive Evidence-Led Research Synthesis for ACP-RA and the Delegated Autonomy Directorate}
...
\renewcommand{\DocVersion}{v11-draft}
\begin{document}
\thispagestyle{empty}
\vspace*{0.6in}
...
\newpage
\begin{execsummary}
\textbf{Purpose.} Establish why delegated autonomy now requires mission-control architecture and institutional command authority.

\textbf{Core finding.} Across C2 reform, swarm operations, interoperability standards, legal governance, adversarial pressure, and infrastructure constraints, the strategic bottleneck is governed delegation throughput under contested conditions.

\textbf{Architecture consequence.} ACP-RA should be treated as command infrastructure with native controls for authority, policy enforcement, provenance, resilience, and rollback.

\textbf{Institutional consequence.} DAD should hold cross-domain authority for scale gates, conformance enforcement, and recurring assurance under operational stress.
\end{execsummary}
...
chronology so each section can be read as a standalone decision input.
...
\hypertarget{integrated-findings-by-topic}{%
\subsection{11) Integrated findings by
topic}\label{integrated-findings-by-topic}}
...
Modern military competition is increasingly defined by how quickly a
force can convert detection into coordinated action while preserving
command responsibility. Across U.S. oversight, competitor adaptation,
and independent analysis, the core pattern is consistent: information
integration and decision architecture now determine operational tempo as
much as platform performance.

GAO's CJADC2 assessment identifies persistent fragmentation, weak
cross-enterprise measurement, and insufficient mechanisms for scaling
lessons learned across organizations. RAND's strategic framework
similarly argues that AI does not eliminate the need for mission command
and that the central problem remains information placement under time
pressure. CSIS analysis of Russian wartime adaptation reinforces this
logic by documenting a shift toward tactical software workflows that
compress targeting timelines and prioritize practical kill-chain speed
over comprehensive, centralized architecture. {[}35{]}{[}38{]}{[}37{]}

For ACP-RA, the implication is direct: command-latency compression must
be engineered under explicit policy and accountability constraints. For
DAD, C2 software integration debt should be governed as a strategic
readiness risk.
...
Swarm dynamics are reshaping force economics by rewarding scale,
attritability, and adaptation speed. This is no longer limited to
experimental programs. It is now visible in procurement priorities,
threat-force research, and field-level sustainment doctrine.

Replicator milestones indicate movement toward high-volume autonomous
fielding and counter-sUAS scaling in compressed timelines. CNA's review
of PRC military writings points to scenario-specific swarm planning,
including Taiwan-relevant assault and blockade operations, alongside
explicit counter-swarm frameworks. CNAS further concludes that
cheap-drone proliferation has democratized precision mass and requires
layered, resilient defenses with faster sensor-to-shooter integration.
U.S. Army sustainment analysis extends the same logic to convoy security
and rear-area defense under manpower constraints.
{[}47{]}{[}56{]}{[}55{]}{[}54{]}{[}53{]}

For ACP-RA, saturation-aware control becomes baseline design, including
prioritization, consequence-tier action gating, and graceful
degradation. For DAD, evaluation and doctrine updates must cover
whole-force application, not only frontline strike formations.
...
Interoperability is rapidly moving from aspiration to engineering
reality. As agents operate across vendors and mission systems, protocol
governance becomes a primary control boundary.

A2A launch and subsequent standardization momentum, including Linux
Foundation stewardship and production-oriented updates, indicate
accelerating convergence around cross-agent communication standards. In
parallel, major platform roadmaps position orchestration controls,
managed runtime behavior, and boundary enforcement as central to
enterprise deployment. OpenAI Frontier's framing aligns with this trend
by identifying deployment governance, context quality, and permissions
discipline as the practical bottlenecks to real-world value.
{[}5{]}{[}51{]}{[}52{]}{[}50{]}{[}1{]}

For ACP-RA, trust should be enforced at message, identity, and connector
boundaries through attestation, provenance, and constrained delegation.
For DAD, interoperability should be conditioned on minimum control
standards before broad federation is authorized.
...
The governance environment is shifting from abstract principle
statements toward implementation frameworks with named responsibility,
lifecycle controls, and verification expectations.

NATO's revised strategy emphasizes responsible-use principles,
interoperability, and verification capacity. The U.S. Political
Declaration process demonstrates expanding state-level dialogue on
operationalization. UK MOD policy and implementation reporting provide
an institutional pattern in which accountable officers, escalation
paths, and lifecycle governance are formalized in organizational
practice. UN General Assembly resolution 79/239 and related UNODA
processes reinforce that international law remains applicable throughout
military AI lifecycles. SIPRI and ICRC analyses continue to anchor the
core point that accountability remains human and institutional, never
machine-borne. {[}40{]}{[}43{]}{[}41{]}{[}42{]}{[}44{]}{[}45{]}{[}46{]}

For ACP-RA, evidence-grade provenance and auditable decision trails are
required design elements for high-consequence operations. For DAD,
scaling authority should remain contingent on demonstrable conformance
to governance and accountability controls.
...
In contested environments, decisive vulnerabilities frequently emerge in
orchestration pathways, software dependencies, and context integrity
rather than in isolated model performance.

OpenClaw ecosystem incidents illustrate practical agent supply-chain
risk and the speed at which compromised extensions can create systemic
exposure. RAND and CSIS analyses reinforce that adversaries adapt
through software-mediated cycles and that durable advantage depends on
iterative adaptation rather than static superiority assumptions. Nuclear
C2 risk analysis further underscores that preserving human launch
authority alone is insufficient if upstream AI-mediated information
flows are brittle or misleading in crisis conditions.
{[}25{]}{[}26{]}{[}27{]}{[}28{]}{[}39{]}{[}37{]}{[}48{]}

For ACP-RA, revocation, isolation, reconstitution, and adversarial
stress testing should be treated as core operational controls. For DAD,
recurrent red-team cycles and crisis-path assurance should be mandatory
...
Autonomy scaling is constrained by infrastructure economics as much as
by algorithmic progress. Compute access, power availability, grid
integration, and financing structure increasingly shape deployment tempo
and strategic resilience.

Reuters reporting on gigawatt-scale facilities and multi-billion-dollar
buildouts indicates that AI capability trajectories are tightly coupled
to capital and infrastructure concentration. Additional reporting on
grid upgrade and utility-cost externalities highlights that deployment
decisions can transfer risk into local systems unless mitigated by
explicit policy and investment commitments. NATO's strategy-level
treatment of compute and energy implications confirms that these are now
alliance-level planning concerns. {[}31{]}{[}32{]}{[}29{]}{[}40{]}

For ACP-RA, architecture should explicitly support tiered compute
envelopes, degraded-mode operation, and edge fallback profiles. For DAD,
autonomy scale decisions should include infrastructure-readiness
criteria and regional resilience considerations as formal gating
factors.
